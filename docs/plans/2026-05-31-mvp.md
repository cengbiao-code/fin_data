...
Create `src/fin_report_extractor/audit_db.py` with schema matching `docs/specs/data-schema.md`. Start with the core tables required by tests:
...
Create `rules/a_share/table_titles.yml`, `concept_aliases.yml`, `unit_patterns.yml`, `period_patterns.yml`, `note_roles.yml`, and `validation_overrides.yml` using the examples from `docs/specs/rule-packs.md`.

- [ ] **Step 4: Create HK and US rule files**

Create equivalent minimal files under `rules/hk` and `rules/us`, using examples from `docs/specs/rule-packs.md`.
