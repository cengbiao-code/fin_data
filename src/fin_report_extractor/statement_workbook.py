...
REQUIRED_LABEL_PATTERNS_BY_MARKET: dict[str, dict[str, list[list[str]]]] = {
    ...
    "us": {
        ...
        "statement.income_statement": [
            ...
            ["earnings per share", "eps", "net income per share", "per share"],
            ["operating income", "income from operations", "income (loss) from operations", "operating profit"],
        ],
        "statement.cash_flow": [
            ["net cash provided by operating activities", "net cash from operating activities", "net cash provided by (used in) operating activities", "cash generated by operating activities", "net cash generated from operating activities"],
            ["net cash used in investing activities", "net cash provided by (used in) investing activities", "cash generated by investing activities", "net cash used in investing activities"],
            ["net cash provided by financing activities", "net cash from financing activities", "net cash provided by (used in) financing activities", "cash used in financing activities", "net cash generated from/(used in) financing activities"],
            ["cash and cash equivalents, end of period", "cash and cash equivalents", "cash and cash equivalents at end of the year", "at end of the year"],
        ],
    },
}
    ...
    *,
    ...
    for role in REQUIRED_STATEMENT_ROLES:
        ...
        for pattern_group in required_patterns.get(role, []):
            matched = False
            # First pass: prefer exact label match (prevents "權益及負債總額"
            # from satisfying a "負債總額" pattern via substring match).
            for pattern in pattern_group:
                normalized_pattern = pattern.strip().lower()
                if any(
                    label.strip().lower() == normalized_pattern
                    for label in labels_list
                ):
                    matched = True
                    break
            # Fallback: substring within each label (handles compound
            # patterns like "經營活動（所用）" inside "經營活動（所用）╱所得").
            # BS labels are always self-contained single concepts, so no
            # substring fallback — prevents "權益及負債總額" from satisfying
            # a "負債總額" pattern.
            if not matched and role != "statement.balance_sheet":
                for pattern in pattern_group:
                    normalized_pattern = pattern.strip().lower()
                    if any(
                        normalized_pattern in label.strip().lower()
                        for label in labels_list
                    ):
                        matched = True
                        break
